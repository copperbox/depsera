...
**Alert rules & history UI:** **[Implemented]** (PRO-108). `AlertRules` component on team detail page with severity filter dropdown (Critical only / Warning and above / All status changes) and enable/disable toggle. Team leads and admins see editable form with save button; team members see read-only summary. `AlertHistory` component displays last 50 alerts in reverse chronological order with columns: time, service, dependency, event type, delivery status (sent/failed/suppressed/muted), and channel type. Status filter dropdown for filtering by delivery status. Handles missing/malformed payloads gracefully. See `client/src/components/pages/Teams/AlertRules.tsx`, `client/src/components/pages/Teams/AlertHistory.tsx`, `client/src/hooks/useAlertRules.ts`, `client/src/hooks/useAlertHistory.ts`.

**Alert delay threshold:** **[Implemented]** (DPS-65). `alert_delay_minutes` column on `alert_rules` (migration 030). Requires a dependency to be continuously unhealthy for N minutes (1-60) before firing an alert. In-memory delay tracking in `AlertService` with `delayTracker` map and `delayAlertSent` set. Recovery events for delayed dependencies that never triggered an alert are also suppressed. Configurable per-team via the alert rules form (independent of custom thresholds checkbox). See `server/src/services/alerts/AlertService.ts` (delay check at step 6 of the pipeline).

**Alert mutes:** **[Implemented]** (DPS-65). `alert_mutes` table (migration 031) with CHECK constraint enforcing exactly one of `dependency_id` (instance mute) or `canonical_name` (canonical mute). Unique indexes prevent duplicate mutes. Optional `expires_at` for time-limited mutes. `AlertMuteStore` with `isEffectivelyMuted()` method checking instance-level then canonical-level mutes, filtering expired entries. Mute check in AlertService pipeline (step 5b) records 'muted' status in alert history. Duration parser utility supports `<number><m|h|d>` format. CRUD API routes: `GET/POST /:id/alert-mutes` (team access/lead), `DELETE /:id/alert-mutes/:muteId` (team lead). Admin read-only list at `GET /admin/alert-mutes`. Expired mutes cleaned up by `DataRetentionService`. Service detail dependency list enriched with `is_muted` boolean. See `server/src/stores/impl/AlertMuteStore.ts`, `server/src/routes/alerts/mutes/`, `server/src/utils/duration.ts`.

**Alert mutes UI:** **[Implemented]** (DPS-65). `AlertMutes` component on team detail page with mute table (type, target, reason, creator, expiry) and add mute form (scope toggle, dependency ID / canonical name input, duration selector, reason). Delete with confirmation dialog. Admin mutes page at `/admin/alert-mutes` for cross-team view. Inline "Muted" badge on service detail dependency rows when `is_muted` is true. See `client/src/components/pages/Teams/AlertMutes.tsx`, `client/src/components/pages/Admin/AlertMutesAdmin.tsx`, `client/src/hooks/useAlertMutes.ts`, `client/src/api/alertMutes.ts`.
