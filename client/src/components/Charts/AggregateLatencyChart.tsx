...
export function AggregateLatencyChart({ dependencyIds, storageKey }: AggregateLatencyChartProps) {
  const [data, setData] = useState<ChartDataPoint[]>([]);
  const [range, setRange] = useState<LatencyRange>('24h');
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Stable key for the dependency IDs to avoid unnecessary re-fetches
  const idsKey = useMemo(() => dependencyIds.slice().sort().join(','), [dependencyIds]);
  ...
  const loadData = useCallback(
    async (selectedRange: LatencyRange) => {
      ...
      setError(null);
      try {
        const response = await fetchAggregateLatencyBuckets(dependencyIds, selectedRange);
        setData(
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h4 className={styles.title}>Aggregate Latency</h4>
        <TimeRangeSelector
