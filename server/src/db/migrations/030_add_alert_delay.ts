...
export function up(db: Database): void {
  db.exec(`ALTER TABLE alert_rules ADD COLUMN alert_delay_minutes INTEGER`);
}
...
export function down(db: Database): void {
  ...
  db.exec(`
    CREATE TABLE alert_rules_backup AS
      SELECT id, team_id, severity_filter, is_active, use_custom_thresholds,
             cooldown_minutes, rate_limit_per_hour, created_at, updated_at
      FROM alert_rules;
